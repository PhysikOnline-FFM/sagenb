...
/*
 * This file is from
 * https://raw.githubusercontent.com/niklasvh/feedback.js/master/feedback.js
 * or
 * https://elearning.physik.uni-frankfurt.de/projekt/browser/elearning-local/feedback
 * 
 * and was modified to adapt the POKAL2 Bootstrap3 look and feel.
 */
...
window.Feedback = function( options ) {
    ...
    returnMethods = {
	    ...
	    // sorry I must inject jquery here because everything else is a stupid message
	    closeButton = $('<button type="button" class="close"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>');
            //  modalHeader.appendChild( a ); // instead:
	    closeButton.click(returnMethods.close).appendTo(modalHeader);
	    
            modalHeader.appendChild( element("h4", options.header ) );
	    // another jquery fixing:
	    $(modalHeader).find("h4").addClass('modal-title');
            ...
            modal.className =  "modal fade feedback-modal";
            modal.setAttribute(H2C_IGNORE, true); // don't render in html2canvas

            // MODAL bootstrap 3 behaviour
	    ...
	    // and now show it (this is Bootstrap3 code!)
	    $(modal).modal();
        },
