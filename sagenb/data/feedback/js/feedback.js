...
// This was adapted for POKAL to match Bootstrap 3 style
...
(function($){
    ...
    var settings = $.extend({
			...
			lineJoin:			'bevel',
			lineWidth:			3,
			html2canvasURL:	'html2canvas.js', // No more needed
		...
		if (supportedBrowser) {
			// den feedbackButton auskommentiert. brauch ich nicht.
			...
			*/
			...
			$('#feedback-module').on('click', '.feedback-wizard-close,.feedback-close-btn, button.close', function() {
				close();
			});
			...
			$(document).on('click', '#feedback-highlighter-back', function() {
				...
				$('#feedback-highlighter').modal('hide'); //hide();
			...
			$(document).on('mousedown', '.feedback-sethighlight', function() {
				highlight = 1;
				$(this).addClass('active'); //feedback-active');
				$('.feedback-setblackout').removeClass('active'); //feedback-active');
			});
			
			$(document).on('mousedown', '.feedback-setblackout', function() {
				highlight = 0;
				$(this).addClass('active');//feedback-active');
				$('.feedback-sethighlight').removeClass('active'); //feedback-active');
			});
			...
			$(document).on('click', '#feedback-overview-back', function(e) {
				canDraw = true;
				$('#feedback-canvas').css('cursor', 'crosshair');
				$('#feedback-overview').modal('hide'); //.hide();
				$('#feedback-helpers').show();
				$('#feedback-highlighter').modal('show'); //.show();
				$('#feedback-overview-error').hide();
			});
			
			// Synchronisierung von erstem und letztem Fenster. Uninteressant
			// da bei uns #feedback-welcome nicht angezeigt wird. 
			...
			$(document).on('click', '#feedback-submit', function() {
				...
				// TODO: Weitere Formulardaten verarbeiten/Worksheet-Inhalte abgreifen!
				if(settings.fillupPost)
					post = settings.fillupPost(post);
